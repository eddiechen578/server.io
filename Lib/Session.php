...
class session
{
    ...
    static function __start()
    {
        ...
        if (session_status() === PHP_SESSION_NONE) {
            if (!$pass) {
                //Warning: ini_set(): Headers already sent. You cannot change the session module's ini settings at this time in myheart.server/code/lib/session.php on line 21
            ...
            //Warning: session_start(): Cannot start session when headers already sent in myheart.server/code/lib/session.php on line 25
            @session_start();
        }
    }
